...
// syncTenantProductsToQdrant sincroniza produtos de um tenant específico para o Qdrant em lote
func (h *ProductHandler) syncTenantProductsToQdrant(tenantID string) error {
	if h.embeddingService == nil {
		return fmt.Errorf("embedding service not available")
	}

	// Buscar produtos apenas deste tenant
	var products []struct {
	...
	if err != nil {
		return fmt.Errorf("failed to fetch products for tenant %s: %w", tenantID, err)
	}

	if len(products) == 0 {
		log.Printf("ℹ️ No products found for tenant %s", tenantID)
		return nil
	}

	log.Printf("📦 Found %d products to sync for tenant %s", len(products), tenantID)
	...
	for _, product := range products {
		...
		batchProduct := services.BatchProductData{
			ID:       product.ID,
	...
	// Recriar collection do tenant para garantir dados atualizados
	// Usar StoreBatchProductEmbeddingsWithRecreate que recria a collection

	batchSize := 500 // Optimized batch size
	err = h.embeddingService.StoreBatchProductEmbeddingsWithRecreate(tenantID, batchProducts, batchSize)
	if err != nil {
		return fmt.Errorf("failed to sync products in batch for tenant %s: %w", tenantID, err)
	}

	log.Printf("✅ Successfully synced %d products for tenant %s", len(batchProducts), tenantID)
	return nil
}
